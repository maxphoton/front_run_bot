"""
Router for market order placement flow (/market command).
Places a market order at the current price and stores it as finished.
"""
...
Use /start to add your first Opinion profile."""
    ...
    if len(accounts) == 1:
        ...
        text = """<tg-emoji emoji-id="5258330865674494479">📊</tg-emoji> Place a Market Order
...
async def process_account_selection(callback: CallbackQuery, state: FSMContext):
    ...
    account_id_str = callback.data.replace("market_select_account_", "")
    try:
    ...
    await callback.message.edit_text(
        """<tg-emoji emoji-id="5258330865674494479">📊</tg-emoji> Place a Market Order
...
async def process_market_url(message: Message, state: FSMContext):
    ...
    if not account_id:
        await message.answer(
            """❌ Account not selected. Please start again with /market."""
        )
        await state.clear()
        return
    ...
    if not account:
        await message.answer(
            """❌ Account not found. Please start again with /market."""
        )
        await state.clear()
        return
    ...
    if is_categorical:
        ...
        for i, subm in enumerate(submarket_list, 1):
            builder.button(
                ...
                callback_data=f"market_submarket_{i}",
            )
        builder.button(text="✖️ Cancel", callback_data="market_cancel")
        builder.adjust(1)
...
• No liquidity on the market"""
    ...
    builder = InlineKeyboardBuilder()
    builder.button(
        text="✅ BUY YES", callback_data="market_side_buy_yes", style="success"
    )
    builder.button(
        text="❌ BUY NO", callback_data="market_side_buy_no", style="success"
    )
    builder.button(
        text="✅ SELL YES", callback_data="market_side_sell_yes", style="danger"
    )
    builder.button(
        text="❌ SELL NO", callback_data="market_side_sell_no", style="danger"
    )
    builder.button(text="✖️ Cancel", callback_data="market_cancel")
    builder.adjust(2)
...
Enter a different amount:""",
    ...
    confirm_text = f"""<tg-emoji emoji-id="5258503720928288433">📋</tg-emoji> <b>Market Order Confirmation</b>

🪙 {direction} {token_name}
Current price: {current_price_str}¢
Amount: {amount} USDT

Place this market order now?"""

    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Place Order", callback_data="market_confirm_yes")
    builder.button(text="✖️ Cancel", callback_data="market_cancel")
    builder.adjust(2)
...
async def process_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    confirm = callback.data.replace("market_confirm_", "")
    if confirm != "yes":
    ...
    await callback.answer()
    await callback.message.edit_text(
        """<tg-emoji emoji-id="5260687119092817530">🔄</tg-emoji> Placing market order..."""
    )

    success, order_id, error_message = await place_market_order(client, order_params)
    ...
    if success:
        try:
            ...
            logger.info(
                "Market order %s saved to DB for account %s", order_id, account_id
            )
        except Exception as exc:
            logger.error("Error saving market order to DB: %s", exc)

        await callback.message.edit_text(
            f"""<tg-emoji emoji-id="5260341314095947411">✅</tg-emoji> <b>Market order placed!</b>
...
• Order ID: <code>{order_id}</code>"""
        ...
        await callback.message.edit_text(
            f"""❌ <b>Failed to place market order</b>
...
@market_order_router.callback_query(F.data == "market_cancel")
async def process_cancel(callback: CallbackQuery, state: FSMContext):
    """Universal cancel handler for /market flow."""
    try:
        await callback.message.edit_text("❌ Order placement cancelled")
    except Exception:
